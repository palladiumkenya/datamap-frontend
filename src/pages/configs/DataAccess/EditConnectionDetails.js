...
const EditConnectionDetails = ({ onNextStep, data }) => {
    ...
    useEffect(() => {
        console.log(data)
        if (data) {
            const [db_type, credentials_host_port_database] = data.split("://");
            const [credentials, host_port_database] = credentials_host_port_database.split('@');
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                    <Box sx={{ display: 'flex', flexDirection: 'column' }}>
                        <Autocomplete
                            ...
                            value={images.find(conn => conn.driver == formData.db_type) || null}
...
export default EditConnectionDetails;
